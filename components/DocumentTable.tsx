...
const statusClass = (s: string) =>
  s === 'completed'
    ...
    : s === 'failed' || s === 'expired'
      ...
      : ['processing', 'queued', 'running', 'uploaded'].includes(s)
    ...
    <div className="card">
      ...
      <div className="table-wrap">
        <table className="table">
          ...
          <tbody>
            {sorted.map((doc) => {
              const s = (STATUS_LABELS[doc.status] ?? doc.status).toLowerCase();
              return (
                <tr key={doc.id}>
                  ...
                  <td className="actions-row">
                    <Link className="btn btn-secondary" href={`/document?documentId=${doc.id}`}>
                      View
                    </Link>
                    {s === 'completed' ? (
                      <Link className="btn btn-primary" href={`/document?documentId=${doc.id}&download=1`}>
                        Download DOCX
                      </Link>
                    ) : null}
                  </td>
